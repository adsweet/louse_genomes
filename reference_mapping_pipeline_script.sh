...
#################################################################################################
#This script will run Bowtie2, filter resulting sites, and produce a consensus from filtered variants
...
bowtie2 --phred33 --local -x $myrefpath/$myref.Genes -U $mypath/READS1,$mypath/READS2 -S $mylib.sam; 
...
#FILTERING
#FILTERING VCF
java -jar PATH/GenomeAnalysisTK.jar -T VariantFiltration -R $myrefpath/$myref.fa -V $mylib.samtools.vcf --filterExpression "DP < 5.0 || DP > 150 || QUAL < 28.0" --filterName "filter2" -o $mylib.filteredDP5.vcf;
